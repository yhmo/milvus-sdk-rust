...
- `Customized` lets the server apply its configured policy.
...
similarity is scored (`L2`, `IP`, `Cosine`, ...). Index creation on the server is asynchronous;
pass `sync` and a timeout when the caller must wait.
