...
pub enum MetricType {
    ...
    #[default]
    Default,
    /// Euclidean distance, `sqrt(sum((a_i - b_i)^2))`. Smaller is more similar.
    L2,
    /// Inner product, `sum(a_i * b_i)`. Larger is more similar.
    Ip,
    /// Cosine similarity, `1 - cos(a, b)`. Larger is more similar.
    Cosine,
    /// Hamming distance, the number of positions at which two binary vectors differ.
    Hamming,
    /// Jaccard similarity between two sets. Larger is more similar.
    Jaccard,
    /// MinHash Jaccard similarity for sparse sets. Larger is more similar.
    MhJaccard,
    /// BM25 relevance score for full-text search over sparse vectors.
    Bm25,
    /// MaxSim Cosine, used to compare dense queries against sparse passage vectors.
    MaxSimCosine,
    /// MaxSim Inner Product, used to compare dense queries against sparse passage vectors.
    MaxSimIp,
    /// MaxSim L2, used to compare dense queries against sparse passage vectors.
    MaxSimL2,
    /// MaxSim Jaccard, used to compare dense queries against sparse passage vectors.
    MaxSimJaccard,
    /// MaxSim Hamming, used to compare dense queries against sparse passage vectors.
    MaxSimHamming,
}
...
pub enum FunctionType {
    ...
    TextEmbedding,
    /// Cross-encoder reranking function applied to a candidate set.
    Rerank,
    /// MinHash function that turns an input field into a sparse MinHash signature.
    MinHash,
}
