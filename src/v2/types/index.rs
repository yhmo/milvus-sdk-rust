...
pub enum IndexType {
    ...
    HnswPq,
    /// HNSW index with product-quantized and vector re-ranking.
    HnswPrq,
    /// Disk-based ANN index that keeps most of the graph on disk.
    DiskAnn,
    /// Lets the server choose an appropriate index automatically.
    AutoIndex,
    /// Disk-friendly scan index with scalar quantization and inverted structure.
    Scann,
